...
var CognitiveStore = class {
	...
	*/
	...
	async load() {
		...
		const [experiences, predictions, tempStrategies, clusters, calibration, channelWeights, exploration, tasks, taxonomy] = await Promise.all([
	...
	*/
	async decideAndExecute(name, decision, situation, threshold = .55, call) {
		...
		return {
			...
			approved,
			executions: await this.loops.requestExecution({
				loopName: name,
				decision,
				situation: `loop:${name} 情境=${situation}`,
				approved,
