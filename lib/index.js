...
var HotEngine = class {
	...
	*/
	channelScores(exp, queryAction, situationVector, queryText, queryEmbedding) {
		return [
			queryEmbedding !== null && exp.embedding !== void 0 ? cosine(queryEmbedding, exp.embedding) : this.scorer.score(queryAction, exp),
			situationVector === null ? 0 : cosine(situationVector, actionVector(exp.sar.situation, [])),
...
var CognitiveStore = class {
	...
	*/
	...
	async load() {
		...
		const [experiences, predictions, tempStrategies, clusters, calibration, channelWeights, exploration, tasks, loopExecutions, acceptance, claimAudits, taxonomy] = await Promise.all([
		...
		if (acceptance !== "") {
			...
			if (Array.isArray(parsed)) for (const record of parsed) {
				...
				this.acceptance.set(check.checkId, {
					...check,
					logVerifiedCount: Number.isInteger(check.logVerifiedCount) && check.logVerifiedCount >= 0 ? check.logVerifiedCount : 0
				});
	...
	* @returns the updated criterion.
	*/
	applyAuditStats(checkId, passed, logVerified = false) {
		const current = this.acceptance.get(checkId);
		if (current === void 0) throw new Error(`cognitive-pipeline: acceptance check "${checkId}" not found`);
		const next = {
			...current,
...
var CognitivePipelineService = class extends Service {
	...
	*/
	async updateAcceptanceCheck(input) {
		...
		const criterion = input.criterion?.trim();
		const evidenceHint = input.evidenceHint?.trim();
		if (criterion === void 0 && evidenceHint === void 0) throw new CognitivePipelineError("cognitive-pipeline: update needs criterion, evidenceHint, or retire", "EMPTY_ACCEPTANCE_UPDATE");
...
function registerPipelineTools(ctx, service) {
	...
	ctx.tools.register(defineTool({
		name: "verify_claim",
		description: "Audit one claim against the active acceptance criteria before treating it as settled. Applicable checks are those whose trigger marker appears in the claim or its situation; a claim with no applicable check audits as not-applicable. An applicable check is satisfied when the claim carries evidence (non-empty), violated when it does not — presence, not truth. When log_anchor is supplied, the session ledger decides instead: the tool reads the executing session's log for the most recent tool/call of that name and checks whether its tool/result matched the expectation — a missing or mismatched anchor violates the claim regardless of self-reported evidence (the log is the witness, so a claim anchored to the ledger cannot be validated by self-report alone). Violated checks accumulate in the criterion ledger, and a criterion whose invoked count clears the evidence minimum while its deviation rate crosses the threshold flags rework_needed and records one deviation meta experience. Pass prediction_id when a predict_outcome exists for the claim: its report_outcome feedback then folds the prediction error into the violated criteria, measuring \"claims without verification\" on the same ruler as every prediction.",
		parameters: {
			claim: {
				type: "string",
				required: true,
			...
			schema: {
				...
				properties: {
					...
					log_verified: {
						type: "boolean",
						required: true,
						description: "True when the audit was backed by a matched session-log anchor (the non-self-referential witness), false for self-reported evidence or no anchor."
					},
					rework_needed: {
						type: "boolean",
						required: true
...
//#endregion
export { ACTION_VECTOR_DIM, CognitiveLoopRegistry, CognitivePipelineService, Config, OUTCOME_VECTOR_DIM, SYMPTOM_MARKERS, UTILITY_SLOTS, actionVector, apply, cosine, findToolCallEvidence, hashToken, inject, isPositiveOutcome, name, normalize, outcomePolarity, outcomeVector, reconstructTurn, refineRetrieval, refineRetrievalFallback, signatureHash, symptomOverlap, tokenize, utilityScore };
