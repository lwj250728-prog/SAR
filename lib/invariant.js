...
const install = (ctx, fail) => {
	...
	for (const check of service.store.acceptanceSnapshot()) {
		...
		if (!Number.isInteger(check.logVerifiedCount) || check.logVerifiedCount < 0 || check.logVerifiedCount > check.passedCount) fail(`acceptance check ${check.checkId} has an invalid log-verified pass count`);
	...
	for (const audit of service.store.claimAuditsSnapshot()) {
		...
		if (audit.logVerified !== (audit.logAnchor !== null && audit.logAnchor.matched)) fail(`claim audit ${audit.auditId} has a log-verification inconsistency`);
	}
};
/**
